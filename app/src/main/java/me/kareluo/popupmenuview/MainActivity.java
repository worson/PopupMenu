...
//        mPopupMenuView = new PopupMenuView(this, R.menu.menu_pop_copy, new MenuBuilder(this));
        mPopupMenuView = new PopupMenuView(this, R.layout.layout_custom_menu);
        MenuBuilder builder=new MenuBuilder(this);
        mPopupMenuView.inflate(R.menu.menu_pop_copy, builder);
        mPopupMenuView.setOnMenuClickListener(this);

        builder.findItem(R.id.menu_collect).setVisible(false);
        for (OptionMenu menuItem : mPopupMenuView.getMenuItems()) {
            if (menuItem.getId()==R.id.menu_collect){
